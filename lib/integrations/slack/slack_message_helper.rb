module Integrations::Slack::SlackMessageHelper
  def process_message_payload
    ...
    FillnodeExceptionTracker.new(e, account: conversation.account).capture_exception
  ...
  def resolve_slack_sender
    ...
    fillnode_user = conversation.account.users.from_email(slack_user[:profile][:email])
    return [fillnode_user, nil, nil] if fillnode_user
