class Notification::PushNotificationService
  ...
  def perform
    ...
    notification_subscriptions.each do |subscription|
      ...
      send_push_via_fillnode_hub(subscription)
    end
  end
  ...
  def handle_browser_push_error(error, subscription)
    ...
    when Errno::ECONNRESET, Net::OpenTimeout, Net::ReadTimeout, Socket::ResolutionError
      ...
      FillnodeExceptionTracker.new(error, account: notification.account).capture_exception
      true
    end
  end
  ...
  def send_push_via_fillnode_hub(subscription)
    return if firebase_credentials_present?
    return unless fillnode_hub_enabled?
    return unless subscription.fcm?

    FillnodeHub.send_push(fcm_options(subscription))
  end
