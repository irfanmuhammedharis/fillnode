...
const termsLink = computed(() =>
  t('REGISTER.TERMS_ACCEPT')
    .replace('https://fillnode.com/terms', globalConfig.value.termsURL)
    .replace(
      'https://fillnode.com/privacy-policy',
      globalConfig.value.privacyURL
    )
);

const allowedLoginMethods = computed(
  () => window.fillnodeConfig.allowedLoginMethods || ['email']
);

const showGoogleOAuth = computed(
  () =>
    allowedLoginMethods.value.includes('google_oauth') &&
    Boolean(window.fillnodeConfig.googleOAuthClientId)
);
