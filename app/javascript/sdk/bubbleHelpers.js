...
export const setBubbleText = bubbleText => {
  if (isExpandedView(window.$fillnode.type)) {
...
export const createBubbleIcon = ({ className, path, target }) => {
  let bubbleClassName = `${className} woot-elements--${window.$fillnode.position}`;
...
const handleBubbleToggle = newIsOpen => {
  ...
  if (newIsOpen) {
    dispatchWindowEvent({ eventName: FILLNODE_OPENED });
  } else {
    dispatchWindowEvent({ eventName: FILLNODE_CLOSED });
    chatBubble.focus();
  }
};
...
export const onBubbleClick = (props = {}) => {
  ...
  const { isOpen } = window.$fillnode;
  if (isOpen === toggleValue) return;

  const newIsOpen = toggleValue === undefined ? !isOpen : toggleValue;
  window.$fillnode.isOpen = newIsOpen;
