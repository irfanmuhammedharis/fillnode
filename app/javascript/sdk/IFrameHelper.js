...
export const IFrameHelper = {
  ...
  createFrame: ({ baseUrl, websiteToken }) => {
    ...
    iframe.id = 'fillnode_live_chat_widget';
    iframe.style.visibility = 'hidden';

    let holderClassName = `woot-widget-holder woot--hide woot-elements--${window.$fillnode.position}`;
    if (window.$fillnode.hideMessageBubble) {
      holderClassName += ` woot-widget--without-bubble`;
    }
    if (isFlatWidgetStyle(window.$fillnode.widgetStyle)) {
  ...
  getAppFrame: () => document.getElementById('fillnode_live_chat_widget'),
  getBubbleHolder: () => document.getElementsByClassName('woot--bubble-holder'),
  sendMessage: (key, value) => {
    const element = IFrameHelper.getAppFrame();
    element.contentWindow.postMessage(
      `fillnode-widget:${JSON.stringify({ event: key, ...value })}`,
      '*'
    );
  },
  initPostMessageCommunication: () => {
    window.onmessage = e => {
      if (
        ...
        e.data.indexOf('fillnode-widget:') !== 0
      ) {
        return;
      }
      const message = JSON.parse(e.data.replace('fillnode-widget:', ''));
  ...
  events: {
    loaded: message => {
      updateAuthCookie(message.config.authToken, window.$fillnode.baseDomain);
      window.$fillnode.hasLoaded = true;
      const campaignsSnoozedTill = Cookies.get('cw_snooze_campaigns_till');
      IFrameHelper.sendMessage('config-set', {
        locale: window.$fillnode.locale,
        position: window.$fillnode.position,
        hideMessageBubble: window.$fillnode.hideMessageBubble,
        showPopoutButton: window.$fillnode.showPopoutButton,
        widgetStyle: window.$fillnode.widgetStyle,
        darkMode: window.$fillnode.darkMode,
        showUnreadMessagesDialog: window.$fillnode.showUnreadMessagesDialog,
        campaignsSnoozedTill,
        welcomeTitle: window.$fillnode.welcomeTitle,
        welcomeDescription: window.$fillnode.welcomeDescription,
        availableMessage: window.$fillnode.availableMessage,
        unavailableMessage: window.$fillnode.unavailableMessage,
        enableFileUpload: window.$fillnode.enableFileUpload,
        enableEmojiPicker: window.$fillnode.enableEmojiPicker,
        enableEndConversation: window.$fillnode.enableEndConversation,
      });
      IFrameHelper.onLoad({
      ...
      if (window.$fillnode.user) {
        IFrameHelper.sendMessage('set-user', window.$fillnode.user);
      }
      ...
      if (!window.$fillnode.resetTriggered) {
        dispatchWindowEvent({ eventName: FILLNODE_READY });
      }
    },
    error: ({ errorType, data }) => {
      dispatchWindowEvent({ eventName: FILLNODE_ERROR, data: data });
    ...
    setBubbleLabel(message) {
      setBubbleText(window.$fillnode.launcherTitle || message.label);
    },

    setAuthCookie({ data: { widgetAuthToken } }) {
      updateAuthCookie(widgetAuthToken, window.$fillnode.baseDomain);
    },

    setCampaignReadOn() {
      updateCampaignReadStatus(window.$fillnode.baseDomain);
    },

    postback(data) {
      dispatchWindowEvent({
        eventName: FILLNODE_POSTBACK,
        data,
      });
    },
  ...
  onLoad: ({ widgetColor }) => {
    ...
    iframe.setAttribute('id', `fillnode_live_chat_widget`);

    if (IFrameHelper.getBubbleHolder().length) {
      return;
    }
    createBubbleHolder(window.$fillnode.hideMessageBubble);
    onLocationChangeListener();

    let className = 'woot-widget-bubble';
    let closeBtnClassName = `woot-elements--${window.$fillnode.position} woot-widget-bubble woot--close woot--hide`;
