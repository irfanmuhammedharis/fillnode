...
const fetchSSLStatus = () => {
  if (!isOnFillnodeCloud.value) return;
...
const handleUpdatePortalConfiguration = portalObj => {
  ...
  // If custom domain is added or updated, fetch SSL status after a delay of 5 seconds (only on Fillnode cloud)
  if (portalObj?.custom_domain && isOnFillnodeCloud.value) {
    setTimeout(() => {
      fetchSSLStatus();
