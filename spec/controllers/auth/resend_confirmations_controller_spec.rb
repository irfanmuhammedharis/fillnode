  ...
  describe 'POST /resend_confirmation' do
    ...
    context 'when hCaptcha is configured' do
      before do
        ...
        allow(FillnodeCaptcha).to receive(:new).and_return(captcha)
      end

      context 'with a valid captcha response' do
        let(:captcha) { instance_double(FillnodeCaptcha, valid?: true) }
      ...
      context 'with an invalid captcha response' do
        let(:captcha) { instance_double(FillnodeCaptcha, valid?: false) }
