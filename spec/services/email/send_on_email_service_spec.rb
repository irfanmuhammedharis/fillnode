...
describe Email::SendOnEmailService do
  ...
  describe '#perform' do
    ...
    context 'when an error occurs' do
      ...
      let(:exception_tracker) { instance_double(FillnodeExceptionTracker, capture_exception: true) }
      ...
      before do
        ...
        allow(FillnodeExceptionTracker).to receive(:new).and_return(exception_tracker)
      end

      it 'captures the exception' do
        expect(FillnodeExceptionTracker).to receive(:new).with(error, account: message.account)
