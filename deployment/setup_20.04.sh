...
# Description: Install and manage a Fillnode installation.
...
FILLNODE_HUB_URL="https://hub.2.fillnode.com/events"
...
function exit_handler() {
  if [ "$?" -ne 0 ] && [ "$u" == "n" ]; then
   echo -en "\nSome error has occured. Check '/var/log/fillnode-setup.log' for details.\n"
   exit 1
  fi
}
...
function get_domain_info() {
  read -rp 'Enter the domain/subdomain for Fillnode (e.g., fillnode.domain.com): ' domain_name
...
serve Fillnode at https://$domain_name.
...
function create_cw_user() {
  if ! id -u "fillnode"; then
    adduser --disabled-password --gecos "" fillnode
  fi
}
...
function save_pgpass() {
  mkdir -p /opt/fillnode/config
  file="/opt/fillnode/config/.pg_pass"
  if ! test -f "$file"; then
    echo $pg_pass > /opt/fillnode/config/.pg_pass
  fi
}
...
##############################################################################
# Configure postgres to create fillnode db user.
...
function configure_db() {
  ...
  sudo -i -u postgres psql << EOF
    ...
    CREATE USER fillnode CREATEDB;
    ALTER USER fillnode PASSWORD :'pass';
    ALTER ROLE fillnode SUPERUSER;
...
function setup_fillnode() {
  ...
  sed -i -e '/POSTGRES_USERNAME/ s/=.*/=fillnode/' .env
...
function run_db_migrations(){
  sudo -i -u fillnode << EOF
  cd fillnode
  RAILS_ENV=production POSTGRES_STATEMENT_TIMEOUT=600s bundle exec rails db:fillnode_prepare
EOF
}

##############################################################################
# Setup Fillnode systemd services and cwctl CLI
# Globals:
#   DEPLOYMENT_TYPE
# Arguments:
#   None
...
function configure_systemd_services() {
  ...
  if [ -f "/etc/systemd/system/fillnode.target" ]; then
  ...
  if [ "$DEPLOYMENT_TYPE" == "web" ]; then
    ...
    if [ "$existing_full_deployment" = true ]; then
      ...
      systemctl stop fillnode.target || true
      systemctl disable fillnode.target || true
      systemctl stop fillnode-worker.1.service || true
      systemctl disable fillnode-worker.1.service || true
    fi

    # Stop and disable worker target if converting from worker-only
    if [ -f "/etc/systemd/system/fillnode-worker.target" ]; then
      echo "Converting from worker-only to web-only"
      systemctl stop fillnode-worker.target || true
      systemctl disable fillnode-worker.target || true
    fi

    cp /home/fillnode/fillnode/deployment/fillnode-web.1.service /etc/systemd/system/fillnode-web.1.service
    cp /home/fillnode/fillnode/deployment/fillnode-web.target /etc/systemd/system/fillnode-web.target

    systemctl daemon-reload
    systemctl enable fillnode-web.target
    systemctl start fillnode-web.target
    ...
    if [ "$existing_full_deployment" = true ]; then
      ...
      systemctl stop fillnode.target || true
      systemctl disable fillnode.target || true
      systemctl stop fillnode-web.1.service || true
      systemctl disable fillnode-web.1.service || true
    fi

    # Stop and disable web target if converting from web-only
    if [ -f "/etc/systemd/system/fillnode-web.target" ]; then
      echo "Converting from web-only to worker-only"
      systemctl stop fillnode-web.target || true
      systemctl disable fillnode-web.target || true
    fi

    cp /home/fillnode/fillnode/deployment/fillnode-worker.1.service /etc/systemd/system/fillnode-worker.1.service
    cp /home/fillnode/fillnode/deployment/fillnode-worker.target /etc/systemd/system/fillnode-worker.target

    systemctl daemon-reload
    systemctl enable fillnode-worker.target
    systemctl start fillnode-worker.target
    ...
    cp /home/fillnode/fillnode/deployment/fillnode-web.1.service /etc/systemd/system/fillnode-web.1.service
    cp /home/fillnode/fillnode/deployment/fillnode-worker.1.service /etc/systemd/system/fillnode-worker.1.service
    cp /home/fillnode/fillnode/deployment/fillnode.target /etc/systemd/system/fillnode.target

    systemctl daemon-reload
    systemctl enable fillnode.target
    systemctl start fillnode.target
  fi

  cp /home/fillnode/fillnode/deployment/fillnode /etc/sudoers.d/fillnode
  cp /home/fillnode/fillnode/deployment/setup_20.04.sh /usr/local/bin/cwctl
...
function setup_ssl() {
  ...
  wget https://raw.githubusercontent.com/fillnode/fillnode/develop/deployment/nginx_fillnode.conf
  cp nginx_fillnode.conf /etc/nginx/sites-available/nginx_fillnode.conf
  certbot certonly --non-interactive --agree-tos --nginx -m "$le_email" -d "$domain_name"
  sed -i "s/fillnode.domain.com/$domain_name/g" /etc/nginx/sites-available/nginx_fillnode.conf
  ln -s /etc/nginx/sites-available/nginx_fillnode.conf /etc/nginx/sites-enabled/nginx_fillnode.conf
...
EOF
  ...
  # Restart the appropriate fillnode target
  if [ -f "/etc/systemd/system/fillnode-web.target" ]; then
    systemctl restart fillnode-web.target
  elif [ -f "/etc/systemd/system/fillnode-worker.target" ]; then
    systemctl restart fillnode-worker.target
  else
    systemctl restart fillnode.target
  fi
}
...
function setup_logging() {
  touch /var/log/fillnode-setup.log
  LOG_FILE="/var/log/fillnode-setup.log"
}
...
***************************************************************************
Woot! Woot!! Fillnode server installation is complete.
...
function cwctl_message() {
  echo $'\U0001F680 Try out the all new Fillnode CLI tool to manage your installation.'
...
function get_cw_version() {
  CW_VERSION=$(curl -s https://app.fillnode.com/api | python3 -c 'import sys,json;data=json.loads(sys.stdin.read()); print(data["version"])')
}
...
***************************************************************************
              Fillnode Installation (v$CW_VERSION)
***************************************************************************

For more verbose logs, open up a second terminal and follow along using,
'tail -f /var/log/fillnode-setup.log'.

EOF

  sleep 3
  read -rp 'Would you like to configure a domain and SSL for Fillnode?(yes or no): ' configure_webserver
  ...
  echo "➥ 6/9 Installing Fillnode. This takes a long while."
  setup_fillnode &>> "${LOG_FILE}"
...
Please refer to the FillNode documentation for domain and SSL configuration.
...
function get_console() {
  sudo -i -u fillnode bash -c " cd fillnode && RAILS_ENV=production bundle exec rails c"
}
...
function help() {
  ...
  cat <<EOF
Usage: cwctl [OPTION]...
Install and manage your Fillnode installation.
...
Installation/Upgrade:
  -i, --install             Install the latest stable version of Fillnode
  -I BRANCH                 Install Fillnode from a git branch
  -u, --upgrade             Upgrade Fillnode to the latest stable version
  -U BRANCH                 Upgrade Fillnode from a git branch (EXPERIMENTAL)
...
Management:
  ...
  -l, --logs                View logs from Fillnode. Supported values include web/worker.
  -r, --restart             Restart Fillnode server
...
Report bugs at https://github.com/fillnode/fillnode/issues
Get help, #
...
##############################################################################
# Get Fillnode web/worker logs (-l/--logs)
# Globals:
#   None
# Arguments:
#   None
...
function get_logs() {
  if [ "$SERVICE" == "worker" ]; then
    journalctl -u fillnode-worker.1.service -f
  fi
  if [ "$SERVICE" == "web" ]; then
    journalctl -u fillnode-web.1.service -f
  fi
}
...
function upgrade_redis() {
  ...
  echo "Upgrading Redis to v7+ for Rails 7 support(Fillnode v2.17+)"
...
##############################################################################
# Install pnpm - this replaces yarn starting from Fillnode 4.0
# Globals:
#   None
# Arguments:
#   None
...
function upgrade() {
  cwctl_upgrade_check
  get_cw_version
  echo "Upgrading Fillnode to v$CW_VERSION (branch: $BRANCH)"
...
EOF
  ...
  if [[ "$(printf '%s\n' "$CW_VERSION" "4.0" | sort -V | head -n 1)" == "4.0" ]]; then
    echo "Fillnode v4.0 and above requires pgvector support in PostgreSQL."
    ...
    if [[ "$user_input" =~ ^([yY][eE][sS]|[yY])$ ]]; then
      ...
      echo "Please install pgvector support before upgrading."
      return 1
    fi
  fi
  ...
  # Navigate to the Fillnode directory
  cd fillnode
...
##############################################################################
# Restart Fillnode server (-r/--restart)
# Globals:
#   None
# Arguments:
#   None
...
function restart() {
  if [ -f "/etc/systemd/system/fillnode-web.target" ]; then
    systemctl restart fillnode-web.target
    systemctl status fillnode-web.target
  elif [ -f "/etc/systemd/system/fillnode-worker.target" ]; then
    systemctl restart fillnode-worker.target
    systemctl status fillnode-worker.target
  else
    systemctl restart fillnode.target
    systemctl status fillnode.target
  fi
}

##############################################################################
# Convert existing Fillnode deployment to different type (--convert)
# Globals:
#   DEPLOYMENT_TYPE
# Arguments:
#   None
...
function convert_deployment() {
  echo "Converting Fillnode deployment to: $DEPLOYMENT_TYPE"

  # Check if Fillnode is installed
  if [ ! -d "/home/fillnode/fillnode" ]; then
    echo "Fillnode installation not found. Use --install first."
    exit 1
  fi
...
function report_event() {
  ...
  curl -X POST -H "Content-Type: application/json" -d "$data" "$FILLNODE_HUB_URL" -s -o /dev/null
}
...
function get_installation_identifier() {
  ...
  installation_identifier=$(sudo -i -u fillnode << "EOF"
  cd fillnode
...
function cwctl_upgrade_check() {
    ...
    local remote_version_url="https://raw.githubusercontent.com/fillnode/fillnode/master/VERSION_CWCTL"
...
function upgrade_cwctl() {
    wget https://get.fillnode.app/linux/install.sh -O /usr/local/bin/cwctl > /dev/null 2>&1 && chmod +x /usr/local/bin/cwctl
}
